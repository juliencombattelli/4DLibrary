...
uLCD_4DSystems::uLCD_4DSystems(PinName tx, PinName rx, PinName reset) : m_serial(tx, rx), m_rst(reset)
{
	clear_rx_buffer();
}
...
void uLCD_4DSystems::clear_rx_buffer()
{
...
void uLCD_4DSystems::write_commande(uint8_t* cmd, uint16_t size)
{
...
void uLCD_4DSystems::read_commande(uint8_t* cmd, uint16_t size)
{
	for(uint8_t i = 0; i < size; i++)
	{
		while (!m_serial.readable());
	...
	/*media function*/
uint16_t uLCD_4DSystems::media_flush()
{
	uint8_t cmd[3];
	cmd[0] = 0x00;
	cmd[1] = 0x8A; 
...
uint8_t uLCD_4DSystems::code_baud_rate(uint32_t baud_rate)
{
	switch(baud_rate)
	{
		...
		case 600: 		return 2; 	break;
		case 1200: 		return 3; 	break;
		case 2400: 		return 4; 	break;
		case 4800: 		return 5; 	break;
		case 9600: 		return 6; 	break;
		case 14400:	 	return 7; 	break;
		case 19200: 	return 8; 	break;
		case 31250: 	return 9; 	break;
		case 38400: 	return 10; 	break;
		case 56000: 	return 11; 	break;
		case 57600: 	return 12;	break;
		case 115200: 	return 13; 	break;
		case 128000: 	return 14; 	break;
		case 256000: 	return 15; 	break;
		case 300000: 	return 16; 	break;
		case 375000: 	return 17; 	break;
		case 500000: 	return 18; 	break;
		case 600000: 	return 19; 	break;
		default: 		return 20;
	}
}
...
*/
	...
	/*txt function*/
uint8_t uLCD_4DSystems::txt_move_cursor(uint16_t line, uint16_t column)
{
	uint8_t cmd[6];
	cmd[0] = 0xFF;
	cmd[1] = 0xE9;
...
uint8_t uLCD_4DSystems::txt_put_char(uint8_t caractere)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xFE;
...
uint8_t uLCD_4DSystems::txt_put_str(uint8_t* str)
{
...
uint16_t uLCD_4DSystems::txt_char_width(uint8_t car)
{
	uint8_t cmd[3];
	cmd[0] = 0x00;
	cmd[1] = 0x1E;
...
uint16_t uLCD_4DSystems::txt_char_height(uint8_t car)
{
	uint8_t cmd[3];
	cmd[0] = 0x00;
	cmd[1] = 0x1D;
...
uint16_t uLCD_4DSystems::txt_foreground_color(uint16_t color)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE7;
...
uint16_t uLCD_4DSystems::txt_background_color(uint16_t color)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE6;
...
uint16_t uLCD_4DSystems::txt_set_font(uint8_t font)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE5;
...
uint16_t uLCD_4DSystems::txt_width_multiplier(uint8_t multiplier)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE4;
...
uint16_t uLCD_4DSystems::txt_height_multiplier(uint8_t multiplier)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE3;
...
uint16_t uLCD_4DSystems::txt_x_gap(uint8_t pixel_count)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE2;
...
uint16_t uLCD_4DSystems::txt_y_gap(uint8_t pixel_count)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xE1;
...
uint16_t uLCD_4DSystems::txt_bold(bool bold)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xDE;
...
uint16_t uLCD_4DSystems::txt_inverse(bool inverse)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xDC;
...
uint16_t uLCD_4DSystems::txt_italic(bool italic)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xDD;
...
uint16_t uLCD_4DSystems::txt_opacity(bool opaque)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xDF;
...
uint16_t uLCD_4DSystems::txt_underline(bool underline) //txt_y_gap command is required to be at least 2 for the underline to be visible
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xDB;
...
uint16_t uLCD_4DSystems::txt_attributes(bool bold, bool italic, bool inverse, bool underline) //txt_y_gap command is required to be at least 2 for the underline to be visible
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xDA;
...
uint16_t uLCD_4DSystems::txt_wrap(uint16_t wrap_pixel)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0xD9;
...
uint8_t uLCD_4DSystems::gfx_draw_polyline(uint16_t n, uint16_t* x_array, uint16_t* y_array, uint16_t colour)
{
	...
	uint32_t cmd_size = 2+array_size*2+2;
	...
	memcpy(&cmd[2], x_array, array_size);
	memcpy(&cmd[2+array_size], y_array, array_size)
...
uint8_t uLCD_4DSystems::gfx_draw_triangle()uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t colour)
{
	...
	/*media function*/
uint16_t uLCD_4DSystems::media_init()
{
	uint8_t cmd[3];
	cmd[0] = 0xFF;
	cmd[1] = 0x89; 
...
uint16_t uLCD_4DSystems::media_set_addr(uint32_t addr)
{
	uint8_t cmd[6];
	cmd[0] = 0xFF;
	cmd[1] = 0x93; 
...
uint16_t uLCD_4DSystems::media_set_sector(uint32_t sector)
{
	uint8_t cmd[6];
	cmd[0] = 0xFF;
	cmd[1] = 0x92; 
...
uint16_t uLCD_4DSystems::media_read_sector(uint8_t* sector)
{
	uint8_t cmd[2];
	cmd[0] = 0x00;
	cmd[1] = 0x16; 
...
uint16_t uLCD_4DSystems::media_write_sector(uint8_t* sector)
{
	uint8_t cmd[3];
	cmd[0] = 0x00;
	cmd[1] = 0x17; 
...
uint16_t uLCD_4DSystems::media_read_byte()
{
	uint8_t cmd[2];
	cmd[0] = 0xFF;
	cmd[1] = 0x8F; 
...
uint16_t uLCD_4DSystems::media_read_word()
{
	uint8_t cmd[2];
	cmd[0] = 0xFF;
	cmd[1] = 0x8E; 
...
uint16_t uLCD_4DSystems::media_write_byte(uint8_t byte)
{
	uint8_t cmd[4];
	cmd[0] = 0x00;
	cmd[1] = 0x8D; 
...
uint16_t uLCD_4DSystems::media_write_word(uint16_t word)
{
	uint8_t cmd[4];
	cmd[0] = 0x00;
	cmd[1] = 0x8C; 
...
uint16_t uLCD_4DSystems::media_display_image(uint16_t x, uint16_t y)
{
	uint8_t cmd[6];
	cmd[0] = 0x00;
	cmd[1] = 0x8B; 
...
uint16_t uLCD_4DSystems::media_display_video(uint16_t x, uint16_t y)
{
	uint8_t cmd[6];
	cmd[0] = 0x00;
	cmd[1] = 0x95; 
...
uint16_t uLCD_4DSystems::media_display_video_frame(uint16_t x, uint16_t y, uint16_t frame_number)
{
	uint8_t cmd[8];
	cmd[0] = 0x00;
	cmd[1] = 0x94; 
	...
	/* UART function*/
uint16_t uLCD_4DSystems::uart_set_baud_rate(uint32_t baud_rate)
{
	uint8_t cmd[4];
	cmd[0] = 0x00;
	cmd[1] = 0x26; 
	...
	/* timer function*/
uint16_t uLCD_4DSystems::timer_sleep_systeme(uint16_t units)
{
	uint8_t cmd[4];
	cmd[0] = 0xFF;
	cmd[1] = 0x3B; 
	...
	/* file function*/
uint16_t uLCD_4DSystems::file_last_error()
{
	uint8_t cmd[3];
	cmd[0] = 0xFF;
	cmd[1] = 0x1F; 
